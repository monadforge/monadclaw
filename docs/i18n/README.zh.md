...
> 详见 [docs/features/auth.md](../features/auth.md)
...
├── crates/
│   ├── agent/          # Agent 会话——对话状态、轮次执行
│   ├── api/            # Axum 路由、handlers、中间件
│   ├── chat/           # 聊天消息类型
│   ├── config/         # TOML 配置加载
│   └── providers/      # LLM Provider trait + genai 后端
...
| LLM Provider trait + genai 后端（OpenAI 兼容） | ✅ 已完成 |
...
| Agent 会话——对话状态、历史、系统 prompt | ✅ 已完成 |
| 有状态轮次执行（`begin_turn` / `commit`） | ✅ 已完成 |
| Agent workspace（SOUL.md、IDENTITY.md、MEMORY.md、bootstrap 流程） | ✅ 已完成 |
| 工具调用（Function Calling） | 🔨 进行中 |
| 短期记忆（服务端 session 持久化） | 🔨 进行中 |
| Discord Bot 接口 | 🔄 计划中 |
| 长期记忆（持久化存储） | 🔄 计划中 |
